...
/**
 * The TETile object is used to represent a single tile in your world. A 2D
 * array of tiles make up a
 * board, and can be drawn to the screen using the TERenderer class.
 * <p>
 * All TETile objects must have a character, textcolor, and background color
 * to be used to represent
 * the tile when drawn to the screen. You can also optionally provide a path
 * to an image file of an
 * appropriate size (16x16) to be drawn in place of the unicode
 * representation. If the image path
 * provided cannot be found, draw will fallback to using the provided
 * character and color
...
public class TETile {
    ...
    // Do not rename character or the autograder will break.
     ...
     * @param description     The description of the tile, shown in the GUI
     *                        on hovering over the tile.
    ...
    /**
     * Constructor without filepath. In this case, filepath will be null, so
     * when drawing, we
    ...
    public TETile(TETile t, Color textColor) {
        this(t.character, textColor, t.backgroundColor, t.description,
             t.filepath);
    }
     ...
     */
    public static TETile colorVariant(TETile t, int dr, int dg, int db,
                                      Random r) {
     ...
     * provided,
     * we draw the image located at that filepath to the screen. Otherwise,
     * we fall
     * back to the character and color representation for the tile.
     * <p>
     * Note that the image provided must be of the right size (16x16). It
     * will not be
    ...
    public void draw(double x, double y) {
        if (filepath != null) {
            try {
                ...
                // Exception happens because the file can't be found. In this
                // case, fail silently
