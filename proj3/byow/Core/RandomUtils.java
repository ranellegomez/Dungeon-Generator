 ...
 * modify this file.
 * <p>
 * Adapted from https://introcs.cs.princeton.edu/java/22library/StdRandom
 * .java.html
 */
public class RandomUtils {
    ...
    public static long uniform(Random random, long n) {
        ...
        // https://docs.oracle.com/javase/8/docs/api/java/util/Random
        // .html#longs-long-long-long-
     ...
     * @throws IllegalArgumentException unless {@code p >= 0.0} and {@code p
     *                                  <= 1.0}
     */
     ...
     * @return a random integer from a Poisson distribution with mean {@code
     * lambda}
     * @throws IllegalArgumentException unless {@code lambda > 0.0} and not
     *                                  infinite
     */
    public static int poisson(Random random, double lambda) {
        ...
        if (Double.isInfinite(lambda)) {
            throw new IllegalArgumentException(
                    "lambda must not be infinite: " + lambda);
        }
        // using algorithm given by Knuth
     ...
     *                                  nearly) equal to 1.0
     * @throws IllegalArgumentException unless {@code probabilities[i] >= 0
     *                                  .0} for each index i
     */
    public static int discrete(Random random, double[] probabilities) {
        ...
        if (sum > 1.0 + eps || sum < 1.0 - eps) {
            throw new IllegalArgumentException(
                    "sum of array entries does not approximately "
                            + "equal 1.0: " + sum);
        }

        // the for loop may not return a value when both r is (nearly) 1.0
        // and when the
     ...
     * @throws IllegalArgumentException if frequencies[i] is negative for any
     *                                  index i
     * @throws IllegalArgumentException if sum of frequencies exceeds Integer
     *                                  .MAX_VALUE (2^31 - 1)
     */
     ...
     * @throws IllegalArgumentException if {@code a} is {@code null}
     * @throws IllegalArgumentException unless {@code (0 <= lo) && (lo < hi)
     *                                  && (hi <= a.length)}
     */
     ...
     * @return an array of length {@code n} that is a uniformly random
     * permutation
    ...
    /**
     * Returns a uniformly random permutation of <em>k</em> of <em>n</em>
     * elements.
     *
     * @param n number of elements
     * @param k number of elements to select
     * @return an array of length {@code k} that is a uniformly random
     * permutation
    ...
    private static void validateSubarrayIndices(int lo, int hi, int length) {
        if (lo < 0 || hi > length || lo > hi) {
            throw new IllegalArgumentException(
                    "subarray indices out of bounds: [" + lo + ", "
                            + hi + ")");
        }
    }
}
